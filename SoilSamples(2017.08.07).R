...
summaryj$importDate<-as.POSIXct(as.numeric(substr(as.character(summaryj$importDate),1,10)),origin = "1970-01-01",tmz="GMT")
summaryj$updatedDate<-as.POSIXct(as.numeric(substr(as.character(summaryj$updatedDate),1,10)),origin = "1970-01-01",tmz="GMT")
...
if("soilSamples"%in%names(soil)){ #Check if there exists soil sample data 
 if(length(soil$soilSamples)>0){ #Another check if there exists soil sample data
   ...
   if(!file.exists(paste("SoilSamples/",summaryj[1,"importFileName"],sep=""))){
   write.csv(soildf,paste("SoilSamples/",summaryj[1,"importFileName"],sep="") )
   }
...
summary<-smartbind(summary,summaryj)
remove(summaryj,s,t,urlsoilkey,soil)
} # Close loop for ALL soil files

summary<-summary[-1,]
row.names(summary)<-summary$importId
summary$publishedDate<-as.POSIXct(as.numeric(substr(as.character(summary$publishedDate),1,10)),origin = "1970-01-01",tmz="GMT")
write.csv(summary,"SoilSamples(2017.08.08).csv")
