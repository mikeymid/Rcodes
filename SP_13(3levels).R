...
colsoil0<-c0Pal(totrow)[as.numeric(cut(c(5.0,FDc0$ph,5.5),breaks=totrow))];colsoil0<-colsoil0[2:(length(colsoil0)-1)];colsoil0
colsoila<-c1Pal(totrow)[as.numeric(cut(c(5.5,FDc1$ph,6.0),breaks=totrow))];colsoila<-colsoila[2:(length(colsoila)-1)];colsoila
colsoilb<-c2Pal(totrow)[as.numeric(cut(c(6.0,FDc2$ph,6.5),breaks=totrow))];colsoilb<-colsoilb[2:(length(colsoilb)-1)];colsoilb
colsoilc<-c3Pal(totrow)[as.numeric(cut(c(6.5,FDc3$ph,7),breaks=totrow))];colsoilc<-colsoilc[2:(length(colsoilc)-1)];colsoilc
colsoild<-c4Pal(totrow)[as.numeric(cut(c(7.0,FDc4$ph,14),breaks=totrow))];colsoild<-colsoild[2:(length(colsoild)-1)];colsoild

#for avoiding error with gradient colors - here we are adding min and max while splitting the colors
#and later removing those values from being displayed

#colsoil0<-c0Pal(totrow)[as.numeric(cut(FDc0$ph,breaks=totrow))];colsoil0
...
#error here - Blue value is 0 in many of the rows
#---------
